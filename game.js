	...
	update(deltaTime) {
		...
		if (this.isEnemyProjectile) {
			...
			for (let module of gameState.modules) {
				const distance = Math.sqrt(
					Math.pow(this.x - module.x, 2) + Math.pow(this.y - module.y, 2),
				);
				if (distance <= 20) {
...
class Module {
	...
	getConnectedComponent(startIndex) {
		...
		while (queue.length > 0) {
			...
			for (let connection of gameState.connections) {
				const otherIndex =
					connection.from === currentIndex
						? connection.to
						: connection.to === currentIndex
							? connection.from
							: null;
	...
	draw() {
		...
		if (this.type === "defense" && this.isConnected && this.droids > 0) {
			...
			if (timeSinceAttack < 200) {
				// Mostrar destello por 200ms después de disparar
				ctx.beginPath();
	...
	update(deltaTime) {
		...
		if (this.type === "production") {
			if (now - this.lastProduction >= 1000) {
				// Cada segundo
		...
		if (this.type === "recruitment") {
			...
			if (
				recruitmentInterval > 0 &&
				now - this.lastRecruitment >= recruitmentInterval
			) {
				if (this.canProduceDroids()) {
	...
	attackEnemies() {
		...
		for (
			let i = 0;
			i < Math.min(attacksThisRound, enemiesInRange.length);
			i++
		) {
...
class Enemy {
	...
	update(deltaTime) {
		...
		for (let module of gameState.modules) {
			const distance = Math.sqrt(
				Math.pow(module.x - this.x, 2) + Math.pow(module.y - this.y, 2),
			);
		...
		if (closestModule) {
			if (closestDistance < 100) {
				// Rango de disparo aumentado
				// Disparar al módulo
				this.target = closestModule;
				if (gameState.gameTime - this.lastAttack >= 1000) {
					// Cada segundo
					const projectile = new Projectile(
						this.x,
						this.y,
...
function updateConnections() {
	...
	for (let { module: energyModule, index: energyIndex } of energyModules) {
		...
		const reachableModules = gameState.modules.filter(
			(module, index) =>
				module.type !== "energy" && !module.isConnected && distances.has(index),
		);
...
function destroyModule(moduleIndex) {
	...
	gameState.connections = gameState.connections
		.filter((conn) => conn.from !== moduleIndex && conn.to !== moduleIndex)
		.map((conn) => ({
...
function placeModule(x, y) {
	...
	for (let module of gameState.modules) {
		const distance = Math.sqrt(
			Math.pow(x - module.x, 2) + Math.pow(y - module.y, 2),
		);
...
function getClickedConnection(x, y) {
	...
	for (let i = 0; i < gameState.connections.length; i++) {
		...
		const lineLength = Math.sqrt(
			Math.pow(moduleB.x - moduleA.x, 2) + Math.pow(moduleB.y - moduleA.y, 2),
		);
		...
		const dotProduct =
			(x - moduleA.x) * (moduleB.x - moduleA.x) +
			(y - moduleA.y) * (moduleB.y - moduleA.y);
...
if (typeof document !== "undefined") {
	canvas.addEventListener("mousedown", (e) => {
		...
		const x =
			gameState.camera.x + (e.clientX - rect.left) / gameState.camera.zoom;
		const y =
			gameState.camera.y + (e.clientY - rect.top) / gameState.camera.zoom;
		...
		if (clickedModule) {
			...
			if (
				...
				if (gameState.selectedConnection !== null) {
					transferDroidByConnection(
						clickedModule,
						gameState.selectedConnection,
					);
				} else {
	...
	function transferDroidByConnection(targetModule, connectionIndex) {
		if (
			targetModule.type === "energy" ||
			targetModule.droids >= targetModule.maxDroids
		)
			return;
		...
		if (
			!sourceModule ||
			sourceModule.type === "energy" ||
			sourceModule.droids === 0
		) {
	...
	function connectionsIntersect() {
		...
		for (let i = 0; i < gameState.connections.length; i++) {
			for (let j = i + 1; j < gameState.connections.length; j++) {
				...
				if (
					!gameState.modules[conn1.from] ||
					!gameState.modules[conn1.to] ||
					!gameState.modules[conn2.from] ||
					!gameState.modules[conn2.to]
				) {
					continue;
				}
				...
				if (
					linesIntersect(m1a.x, m1a.y, m1b.x, m1b.y, m2a.x, m2a.y, m2b.x, m2b.y)
				) {
					return true;
				}
			}
		...
		for (let i = 0; i < gameState.connections.length; i++) {
			...
			for (let j = 0; j < gameState.modules.length; j++) {
				...
				if (
					lineIntersectsCircle(ma.x, ma.y, mb.x, mb.y, module.x, module.y, 20)
				) {
					return true;
				}
			}
	...
	function updateUI() {
		...
		document.getElementById("resources").textContent = Math.floor(
			gameState.resources,
		);
		document.getElementById("droids").textContent =
			`${assignedDroids}/${gameState.totalDroids}`;
		...
		document.getElementById("energy").textContent =
			`${usedModules}/${totalEnergyCapacity}`;
		document.getElementById("wave").textContent = gameState.waveNumber;
		document.getElementById("gameSpeedDisplay").textContent =
		...
		if (pauseButton) {
			pauseButton.textContent = gameState.gamePaused
				? "Reanudar Juego"
				: "Pausar Juego";
			pauseButton.style.background = gameState.gamePaused
				? "#28a745"
				: "#0f3460";
		}
		...
		buildingButtons.forEach(({ id, type, color }) => {
			...
			if (button && moduleTypes[type]) {
				...
				const costText =
					connectionCost > 0 ? `${cost}+${connectionCost}` : cost.toString();
	...
	function render() {
		...
		if (gameState.gamePaused) {
			...
			ctx.fillText(
				'Presiona ESPACIO o click en "Reanudar" para continuar',
				canvas.width / 2,
				canvas.height / 2 + 60,
			);
		}
	...
	function updateCamera(deltaTime) {
		...
		const speed =
			(gameState.camera.speed * deltaTime) / 1000 / gameState.camera.zoom;
	...
	function gameLoop(currentTime) {
		...
		// Generar oleadas
		if (
			gameState.gameTime >= gameState.nextWaveTime &&
